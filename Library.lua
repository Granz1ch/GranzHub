-- Granz Hub Advanced Library v2.0
...
ScreenObject.Name = "GranzHub_Environment"
...
if ParentGui:FindFirstChild("GranzHub_Environment") then
	for _, v in pairs(ParentGui:GetChildren()) do
		if v.Name == "GranzHub_Environment" and v ~= ScreenObject then
			v:Destroy()
		end
	end
end
...
-- Notification System
function GranzHub:Notify(options)
...
function GranzHub:CreateWindow(options)
	local WindowName = options.Name or "Granz Hub"
	...
	task.spawn(function()
		for i = 1, 30 do
			...
			-- Float animation
			local targetY = p.Position.Y.Scale + (math.random() > 0.5 and -0.15 or 0.15)
			local targetX = p.Position.X.Scale + (math.random() - 0.5) * 0.1
			Utility:Tween(p, {math.random(30, 60) / 10, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut}, {
				Position = UDim2.new(math.clamp(targetX, 0.02, 0.98), 0, math.clamp(targetY, 0.02, 0.98), 0),
	...
	GlowRing.Parent = IntroOverlay
	GlowRing.AnchorPoint = Vector2.new(0.5, 0.5)
	GlowRing.Position = UDim2.new(0.5, 0, 0.45, 0)
	...
	GlowRingStroke.Color = Color3.fromRGB(255, 255, 255)
	GlowRingStroke.Thickness = 2
	GlowRingStroke.Transparency = 1
	Instance.new("UICorner", GlowRing).CornerRadius = UDim.new(1, 0)

	-- Inner glow ring
	local GlowRing2 = Instance.new("Frame")
	GlowRing2.Parent = IntroOverlay
	GlowRing2.AnchorPoint = Vector2.new(0.5, 0.5)
	GlowRing2.Position = UDim2.new(0.5, 0, 0.45, 0)
	GlowRing2.Size = UDim2.new(0, 0, 0, 0)
	GlowRing2.BackgroundTransparency = 1
	GlowRing2.ZIndex = 101
	GlowRing2.BorderSizePixel = 0

	local GlowRingStroke2 = Instance.new("UIStroke")
	GlowRingStroke2.Parent = GlowRing2
	GlowRingStroke2.Color = Color3.fromRGB(180, 180, 200)
	GlowRingStroke2.Thickness = 1
	GlowRingStroke2.Transparency = 1
	Instance.new("UICorner", GlowRing2).CornerRadius = UDim.new(1, 0)

	-- Logo / brand text "G" emblem
	local LogoLabel = Instance.new("TextLabel")
	LogoLabel.Parent = IntroOverlay
	...
	IntroTitle.Position = UDim2.new(0.5, 0, 0.62, 0)
	IntroTitle.Size = UDim2.new(0, 400, 0, 50)
	IntroTitle.Font = Enum.Font.GothamBold
	IntroTitle.Text = "GRANZ HUB"
	...
	IntroSub.Position = UDim2.new(0.5, 0, 0.68, 0)
	IntroSub.Size = UDim2.new(0, 400, 0, 24)
	IntroSub.Font = Enum.Font.Ubuntu
	IntroSub.Text = "Excellence Redefined"
	...
	LoadBarBG.Parent = IntroOverlay
	LoadBarBG.AnchorPoint = Vector2.new(0.5, 0.5)
	LoadBarBG.Position = UDim2.new(0.5, 0, 0.76, 0)
	LoadBarBG.Size = UDim2.new(0, 220, 0, 3)
	...
	LoadStatus.Parent = IntroOverlay
	LoadStatus.BackgroundTransparency = 1
	LoadStatus.AnchorPoint = Vector2.new(0.5, 0.5)
	LoadStatus.Position = UDim2.new(0.5, 0, 0.80, 0)
	LoadStatus.Size = UDim2.new(0, 300, 0, 18)
	LoadStatus.Font = Enum.Font.Ubuntu
	LoadStatus.Text = ""
	LoadStatus.TextColor3 = Color3.fromRGB(100, 100, 115)
	LoadStatus.TextSize = 11
	LoadStatus.TextTransparency = 1
	LoadStatus.ZIndex = 103

	-- Decorative corner lines
	local function MakeCornerLine(posX, posY, sizeW, sizeH, ancX, ancY)
		local line = Instance.new("Frame")
		line.Parent = IntroOverlay
		line.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
		line.BackgroundTransparency = 1
		line.AnchorPoint = Vector2.new(ancX, ancY)
		line.Position = UDim2.new(posX, 0, posY, 0)
		line.Size = UDim2.new(0, sizeW, 0, sizeH)
		line.BorderSizePixel = 0
		line.ZIndex = 102
		return line
	end

	local cTL1 = MakeCornerLine(0.05, 0.08, 40, 1, 0, 0)
	local cTL2 = MakeCornerLine(0.05, 0.08, 1, 40, 0, 0)
	local cTR1 = MakeCornerLine(0.95, 0.08, 40, 1, 1, 0)
	local cTR2 = MakeCornerLine(0.95, 0.08, 1, 40, 1, 0)
	local cBL1 = MakeCornerLine(0.05, 0.92, 40, 1, 0, 1)
	local cBL2 = MakeCornerLine(0.05, 0.92, 1, 40, 0, 1)
	local cBR1 = MakeCornerLine(0.95, 0.92, 40, 1, 1, 1)
	local cBR2 = MakeCornerLine(0.95, 0.92, 1, 40, 1, 1)
	local cornerLines = {cTL1, cTL2, cTR1, cTR2, cBL1, cBL2, cBR1, cBR2}
	...
	task.spawn(function()
		...
		for _, cl in ipairs(cornerLines) do
			Utility:Tween(cl, {0.6, Enum.EasingStyle.Quart, Enum.EasingDirection.Out}, {BackgroundTransparency = 0.5})
		end
		task.wait(0.3)

		-- Phase 3: Expanding glow rings
		Utility:Tween(GlowRingStroke, {0.5}, {Transparency = 0.4})
		Utility:Tween(GlowRing, {1.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out}, {Size = UDim2.new(0, 140, 0, 140)})
		task.wait(0.15)
		Utility:Tween(GlowRingStroke2, {0.5}, {Transparency = 0.6})
		Utility:Tween(GlowRing2, {1.4, Enum.EasingStyle.Quart, Enum.EasingDirection.Out}, {Size = UDim2.new(0, 200, 0, 200)})

		-- Phase 4: Logo "G" fades in with scale
		LogoLabel.Size = UDim2.new(0, 60, 0, 60)
		Utility:Tween(LogoLabel, {0.8, Enum.EasingStyle.Back, Enum.EasingDirection.Out}, {
			TextTransparency = 0,
			Size = UDim2.new(0, 100, 0, 100)
		})
		task.wait(0.5)

		-- Phase 5: Title slides up and fades in
		IntroTitle.Position = UDim2.new(0.5, 0, 0.66, 0)
		Utility:Tween(IntroTitle, {0.8, Enum.EasingStyle.Quart, Enum.EasingDirection.Out}, {
			TextTransparency = 0,
		...
		Utility:Tween(IntroSub, {0.6, Enum.EasingStyle.Quart, Enum.EasingDirection.Out}, {TextTransparency = 0})
		task.wait(0.3)

		-- Phase 7: Loading bar appears and fills
		Utility:Tween(LoadBarBG, {0.3}, {BackgroundTransparency = 0})
		Utility:Tween(LoadBarFill, {0.3}, {BackgroundTransparency = 0})
		Utility:Tween(LoadStatus, {0.3}, {TextTransparency = 0})

		local loadSteps = {
			{progress = 0.15, text = "Initializing modules..."},
			{progress = 0.35, text = "Loading interface..."},
			{progress = 0.55, text = "Preparing components..."},
			{progress = 0.75, text = "Connecting services..."},
			{progress = 0.90, text = "Almost ready..."},
			{progress = 1.00, text = "Welcome to Granz Hub"},
		}

		for _, step in ipairs(loadSteps) do
			LoadStatus.Text = step.text
			Utility:Tween(LoadBarFill, {0.4, Enum.EasingStyle.Quart, Enum.EasingDirection.Out}, {
		...
		-- Logo pulses and fades
		Utility:Tween(LogoLabel, {0.6}, {
			TextTransparency = 1,
			Size = UDim2.new(0, 140, 0, 140)
		})
		Utility:Tween(IntroTitle, {0.5}, {TextTransparency = 1})
		Utility:Tween(IntroSub, {0.5}, {TextTransparency = 1})
		Utility:Tween(LoadBarBG, {0.4}, {BackgroundTransparency = 1})
		Utility:Tween(LoadBarFill, {0.4}, {BackgroundTransparency = 1})
		Utility:Tween(LoadStatus, {0.4}, {TextTransparency = 1})
		Utility:Tween(GlowRingStroke, {0.5}, {Transparency = 1})
		Utility:Tween(GlowRingStroke2, {0.5}, {Transparency = 1})

		for _, cl in ipairs(cornerLines) do
			Utility:Tween(cl, {0.4}, {BackgroundTransparency = 1})
		end

		-- Fade out particles
		for _, p in ipairs(particleFrames) do
			if p and p.Parent then
				Utility:Tween(p, {0.3}, {BackgroundTransparency = 1})
			end
		end

		task.wait(0.7)

		-- Phase 9: Show main UI
		...
		fadeBg.Completed:Connect(function()
			...
			WindowOpen = true
			GranzHub:Notify({Title = "Welcome", Content = "Granz Hub loaded successfully.", Duration = 4})
		end)
	end)
	...
	local function ToggleUI(state)
		...
		if WindowOpen then
			...
			GranzHub:Notify({Title = "UI Hidden", Content = "Press INSERT to open the menu again.", Duration = 4})
		end
	end
	...
	function Window:CreateTab(tabName)
		...
		function Elements:CreateSlider(options)
			...
			local function fmt(v)
				if isFloat then return string.format("%." .. decimals .. "f", v) end
